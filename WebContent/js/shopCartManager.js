			    	...
			    	for(var i in cartSelectedmerMap){
			    		...
			    		str += "<tr class='merSelect' class='text' align='center' bgcolor='#FFFFFF'>"+						
			    		         "<td>&nbsp;<a href='mer.do?method=showMer&id="+ cartSelected.merchandise +"' target='_blank'> "+
							          "<span class='blueText'>"+ cartSelected.merName +"</span></a>"+
								"</td>"+
								"<td>￥"+ cartSelected.merPrice +"</td>"+
								"<td>￥<span id='price81'>"+ cartSelected.cartSelectedMerPrice +"</span></td>"+
								"<td><input type='text' class='textBox' onChange='modiNum(this.value)' value="+ cartSelected.number +" size='4'/></td>"+
								"<td>￥<span id='money81'>"+ cartSelected.cartSelectedMerMoney +" </span></td>"+
								"<td><input onClick='deleteCart("+cartSelected.id + "," + cartSelected.cart +")' type='image' src='/EShop/img/delete_01.gif' border='0'/></td>"+
						    "</tr>";
			    	}
...
function clearCart(){
	var url = "shopCartManager?method=delete&merId="+merId+"&fun=0"+"&cartId="+cartId;
	ajax(url,function(){
		if(xhr.readyState == 4 && xhr.status == 200){
			if("fail" != xhr.response){
				alert("商品删除成功！");
			    ...
			    if(arr.length != 0){
			    	for(var i in cartSelectedmerMap){
			    		...
			    		str += "<tr class='merSelect' class='text' align='center' bgcolor='#FFFFFF'>"+						
								...
								"<td>￥"+ cartSelected.merPrice +"</td>"+
								"<td>￥<span id='price81'>"+ cartSelected.cartSelectedMerPrice +"</span></td>"+
								"<td><input type='text' class='textBox' onChange='modiNum(this.value,"+cartSelected.id + "," + cartSelected.cart +")' value="+ cartSelected.number +" size='4'/></td>"+
								"<td>￥<span id='money81'>"+ cartSelected.cartSelectedMerMoney +" </span></td>"+
								"<td><input onClick='deleteCart("+cartSelected.id + "," + cartSelected.cart +")' type='image' src='/EShop/img/delete_01.gif' border='0'/></td>"+
						    "</tr>"
			    	}
